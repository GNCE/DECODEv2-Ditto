...
public class PinpointVisionLocalizer implements Localizer {
    ...
    private PoseEKF ekf;
    private double lastVMTimestamp;
     ...
     */
    public PinpointVisionLocalizer(HardwareMap map, PinpointConstants constants){ this(map, constants, new Pose());}
     ...
     */
    @SuppressLint("NewApi")
    public PinpointVisionLocalizer(HardwareMap map, PinpointConstants constants, Pose setStartPose){
        ekf = new PoseEKF();
        lastVMTimestamp = 0;
    ...
    public void setStartPose(Pose setStart) {
        if (!Objects.equals(startPose, new Pose()) && startPose != null) {
            Pose currentPose = fusionPose.rotate(-startPose.getHeading(), false).minus(startPose);
    ...
    public void update() {
        ...
        ekf.predictFromAbsoluteOdo(now, currentPinpointPose, speed, omega);
        VisionMeasurement vm = MyRobot.vm;
        if(vm != null && vm.timestampSec > lastVMTimestamp + 1e-6){
            ekf.updateVisionWithLatency(now, vm.pose, vm.timestampSec, speed, omega, vm.quality);
            lastVMTimestamp = vm.timestampSec;
        }

        Pose currentFusionPose = ekf.getPose();

        totalHeading += MathFunctions.getSmallestAngleDifference(currentFusionPose.getHeading(), previousHeading) * MathFunctions.getTurnDirection(previousHeading, currentFusionPose.getHeading());
        previousHeading = currentFusionPose.getHeading();
