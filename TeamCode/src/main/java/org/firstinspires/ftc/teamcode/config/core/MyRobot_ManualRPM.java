...
@Configurable
public class MyRobot_ManualRPM extends Robot {
    HardwareMap h;
...
// ---- END SOTM DEBUG TELEMETRY ----
    ...
    public void runShootTeleop(){
        ...
        // MANUAL RPM PRE-SPIN (g2 bumpers): hold g2 LEFT BUMPER to spin the flywheel up to the
        // CLOSE baseline (CLOSE_PRESPIN_RPM); hold g2 RIGHT BUMPER for the FAR baseline
        // (FAR_PRESPIN_RPM). From there the normal per-loop distance logic still runs, so the
        // wheel only fine-tunes around the baseline instead of ramping from idle. Releasing either
        // bumper (when neither is held) clears the hold and hands the wheel back to the live
        // distance target. Firing is still gated on shooter.readyToShoot() via OuttakeCommandTele.
        if(hasSubsystem(SubsystemConfig.SHOOTER)){
